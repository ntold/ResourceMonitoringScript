...
THRESHOLD=$1				#Die Schwelle um die Email zu verschiken
...
do
	...
	#-------------------------------------------------------[ Get PID and %CPU ]-------------------------------------------------------#
	#In diedem Abschnitt werden die derzeitigen Prozente und deren Prozess ID ausgelesen
	#Wenn mas das mit grep, sort & cut macht wird das ganze in einem String gespiechert in der Folgenden Reihenfolge: PID %CPU PID %CPU PID ... (jedes Paar 10 Mal)
	...
	do
		if [[ ${ARR_PERCENTAGES[$i]} -gt $THRESHOLD && ${ARR_PID_CHECK[*]} != *${ARR_PID[$i]}* ]]; then	#Wenn die Prozentzahl grösser ist als die Schwelle UND die Prozess ID noch nicht im Array (ARR_PID_CHECK) ist
			...
			$EMAIL_LISTENER "${ARR_PID[$i]}" &			#Starte den Eamil listener
